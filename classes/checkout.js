...
class checkout {
    ...
    async main(){
        if (this.page == null){
            this.page = await checkout.browser.newPage();
        ...
        if( store.get('getProgressState') == false) {
            //await this.page.goto('https://www.amazon.com/gp/offer-listing/B08615TQ9C/ref=dp_olp_unknown_mbc' ,{ waitUntil: 'domcontentloaded' }); //test
            try{
    ...
    async dp(){
        ...
        if(  this.item.price  >= parseFloat(price)) {
            ...
            logger.warn('add to cart step 1');
            try{
            ...
            const date = new Date().toLocaleString();
            console.log(`[${date}]`.green+`[Info]`+ ` ${this.item.name} price ${parseFloat(price)} does not meet the set price ${this.item.price}`);
            if(config.offerList==1)
                await this.main2();
        }
    }
